...
namespace sict
{
	...
	{
		file << m_type << "," << m_sku << "," << m_name << "," << m_price << "," << m_taxable << "," << m_qty << "," << m_unit << "," << m_qtyNeeded;
	...
	std::ostream& NonPerishable::write(std::ostream& os, bool linear) const
	{
		if (m_errorState.isClear())
		{
			if (linear)
			{
				...
				os << setw(4) << m_qty << "|";
				os << setw(10) << left << m_unit << "|";
				os << setw(4) << right <<  m_qtyNeeded << "|";
			}
			else
			{
				os << "Sku: " << m_sku << endl;
				os << "Name: " << m_name << endl;
				os << "Price: " << fixed << setprecision(2) << m_price << endl;
				if (m_taxable)
				{
